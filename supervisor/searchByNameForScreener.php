...
$pic = $_SESSION["pic"];
		...
		if(isset($_GET["btn"])){
			...
			while ($row=$result->fetch_assoc()) {
				...
				$User.="<tr><td class='center'><img src='../pics/".$row["Photo"]."' height='140' width='180'></td><td class='center'>".$row["IOM"].
				"</td><td class='center'>".$row["UAM"]."</td><td class='center'>".$row["FirstName"]." ".$row["LastName"].
				" </td><td class='center'><a href='detailedDisplayOfEntrySup.php?Id=".$row["ChildId"]."'>View</a></td></tr>";

			}

		$User.="</table>";
	}else{
		$msg = "<p style='text-align:center;'><font color='red'> No Records Found! </font></p>";
	}
}
}
		if(isset($_GET["rst"])){
